 ...
 */
Template.sortSqueaksDropdown.created = function() { 
  Session.setDefault('squeakListSorting', 'v');
}
/**
 * If we're in tutorial mode, kill the links
 */
Template.sortSqueaksDropdown.rendered = function() { 
  if (Session.equals('isTutorial', true)) { $('.squeak-list-sorting-link').addClass('disabled'); }
}
/**
 ...
 */
Template.sortSqueaksDropdown.helpers({
  current: function() { 
    return dictionary[Session.get('squeakListSorting')];
  },
  /**
  ...
  needToDisplay: function(that) { 
    return !Session.equals('squeakListSorting', that);
  },
  /** 
 ...
 * @author  moore
 */
Template.sortSqueaksDropdown.events({
  /**
   * Clicking on one of the links should just set the Session var accordingly
   * 
   */
  'click .squeak-list-sorting-link': function(event) { 
    event.preventDefault();
    
    return Session.set('squeakListSorting', rev[$(event.currentTarget).text().trim()]);
  }
})